...
PatchWorkpp<PointT>::PatchWorkpp(ros::NodeHandle *nh) : node_handle_(*nh) {
    // Init ROS related
...
template<typename PointT> inline
PatchWorkpp<PointT>::~PatchWorkpp(){
}
template<typename PointT> inline
        ...
        for (int ring_idx = 0; ring_idx < num_rings_each_zone_[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx]; ++sector_idx) {
                ...
                /*  
                    ...
                    (Normal Vector is heading outside if it is heading away from the origin)
                ...
                */
                ...
                else if (!is_heading_outside && is_near_zone)
                {
                    cloud_nonground += regionwise_ground_;
                }
...
template class PatchWorkpp<pcl::PointXYZI>;
template class PatchWorkpp<PointXYZILID>;
}